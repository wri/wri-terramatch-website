export type DashboardServiceFetcherExtraProps = {
  /**
   * You can add some extra props to your generated fetchers.
   *
   * Note: You need to re-gen after adding the first property to
   * have the `DashboardServiceFetcherExtraProps` injected in `DashboardServiceComponents.ts`
   **/
};
...
export type DashboardServiceFetcherOptions<TBody, THeaders, TQueryParams, TPathParams> = {
  url: string;
  method: string;
  body?: TBody;
  headers?: THeaders;
  ...
  signal?: AbortSignal;
} & DashboardServiceFetcherExtraProps;

export async function dashboardServiceFetch<
  TData,
  TError,
...
>({
  ...
  queryParams,
  signal
}: DashboardServiceFetcherOptions<TBody, THeaders, TQueryParams, TPathParams>): Promise<TData> {
  try {
