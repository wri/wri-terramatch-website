export type ResearchServiceFetcherExtraProps = {
  /**
   * You can add some extra props to your generated fetchers.
   *
   * Note: You need to re-gen after adding the first property to
   * have the `ResearchServiceFetcherExtraProps` injected in `ResearchServiceComponents.ts`
   **/
};
...
export type ResearchServiceFetcherOptions<TBody, THeaders, TQueryParams, TPathParams> = {
  url: string;
  method: string;
  body?: TBody;
  headers?: THeaders;
  ...
  signal?: AbortSignal;
} & ResearchServiceFetcherExtraProps;

export async function researchServiceFetch<
  TData,
  TError,
...
>({
  ...
  queryParams,
  signal
}: ResearchServiceFetcherOptions<TBody, THeaders, TQueryParams, TPathParams>): Promise<TData> {
  try {
