 ...
 *
 * `field` is reserved for a WRI ground survey and must not be borrowed for anything else. A
 * polygon's area is computed from the submitted boundary, which is neither a field measurement nor
 * a remote one, so it gets its own `geometry` tier. Letting it say "Field measured" would collide
 * with the vocabulary the data team uses for the field tier.
 */
export type SourceTier = "field" | "geometry" | "satellite" | "modeled" | "reported" | "derived" | "notMeasured";

export const SOURCE_LABELS: Record<SourceTier, string> = {
  field: "Field measured",
  geometry: "From boundary",
...
export const LEVEL_CONTRACT: Record<IndicatorKey, IndicatorContract> = {
  hectares: {
    ...
    source: { project: "derived", site: "derived", polygon: "geometry" },
  ...
  treeCount: {
    key: "treeCount",
    // Not "Tree count (measured)". With a partner claim rendered on this row, the parenthetical is
    // wrong — the row now carries both a claim and (eventually) a measurement, and the chips say
    // which is which.
    label: "Tree count",
    unit: "trees",
    hasData: false,
