...
export interface SemanticZoom {
  ...
  claims?: Partial<Record<IndicatorKey, { value: number | null; label: string }>>;
  reconciliations?: Partial<Record<IndicatorKey, Reconciliation | null>>;
  goals?: Partial<Record<IndicatorKey, number | null>>;
  /** Why an indicator has no measurement yet, keyed by indicator. */
  expectations?: Partial<Record<IndicatorKey, string>>;
  /** The level shown before this one, so the panel can say what just changed. Null on first view. */
...
export const useSemanticZoom = ({ projectUuid, projectName, claims, goals }: UseSemanticZoomOptions): SemanticZoom => {
  ...
  // Remembering the level we came from is what lets the panel state what just changed. Tracked in
  // refs rather than state because it must not itself trigger a render — the strip is a
  // description of a transition that has already happened.
  const currentLevelRef = useRef<Level>(level);
  const previousLevelRef = useRef<Level | null>(null);
  if (currentLevelRef.current !== level) {
    previousLevelRef.current = currentLevelRef.current;
  ...
  // Inside a site the polygons themselves are often sub-pixel: a smallholder site can scatter 70
  // parcels of a hectare or two across 80km, and at the zoom that frames all of them each one is a
  // fraction of a pixel. The shapes are still the truth, so they stay — but each also gets a marker
  // at its centroid, sized by zoom, so there is something visible to see and to paint when the loss
  // timeline runs. The markers fade out as the real geometry becomes legible.
  const polygonMarkers = useMemo(() => {
    if (siteUuid == null || featureCollection == null) return [];

    return featureCollection.features
      .map(feature => {
        const uuid = feature.properties?.uuid;
        if (typeof uuid !== "string") return null;
        if (feature.geometry == null || !("coordinates" in feature.geometry)) return null;
        ...
        return {
          type: "Feature" as const,
          // Same uuid as the polygon it stands for, so one setFeatureState paints both and a click
          // on either resolves to the same selection.
          properties: { uuid, siteId: siteUuid, kind: "polygonMarker" },
          geometry: { type: "Point" as const, coordinates: [lng / n, lat / n] }
        };
      })
      .filter(feature => feature != null);
  }, [siteUuid, featureCollection]);

  const siteLevelFeatures = useMemo(() => {
    if (siteUuid == null || featureCollection == null) return featureCollection;
    return {
      type: "FeatureCollection",
      features: [...featureCollection.features, ...polygonMarkers]
    } as FeatureCollection;
  }, [siteUuid, featureCollection, polygonMarkers]);
  ...
  // Hectares deliberately has no reconciliation. The project's "reported" hectares figure is
  // SUM(calc_area) over the same approved, active, non-deleted polygons the rollup sums — the same
  // column reached by two code paths. Comparing them printed "Measured matches reported" in every
  // case, which is a tautology wearing the costume of a check, and it discredits the coverage
  // lines and provenance chips around it. Hectares is geometry; it has no partner claim to check.
  // Reconciliation returns when a measured tree count exists to check the reported one against.
  const reconciliations = undefined;
  ...
  return {
    ...
    aggregate,
    childEntries,
    // Only tree count carries a claim, and only at project level. Partners report planting in site
    // reports, so the figure is native to the reporting hierarchy, not to a polygon — there is no
    // honest way to show it further down. The label names the instrument rather than asserting a
    // status: it is what a partner filed, not something anyone has checked.
    claims:
      level === "project" && claims?.trees != null
        ? { treeCount: { value: claims.trees, label: "Reported (site reports)" } }
        : undefined,
    reconciliations,
    goals: level === "project" ? { hectares: goals?.hectares ?? null, treeCount: goals?.trees ?? null } : undefined,
    expectations:
      level === "project" && claims?.trees != null
        ? {
            // No date. The two-year expectation rule has no policy sign-off, and printing an
            // invented "Jun 2028" would break the same provenance rule this panel enforces
            // everywhere else. The mechanism is stateable today; the schedule is not.
            treeCount:
              "No independent measurement yet to check this figure against. Reconciliation starts with the first satellite tree count."
          }
        : undefined,
    previousLevel: previousLevelRef.current,
    mapFeatures: siteCentroids ?? siteLevelFeatures,
    mapLoading,
    rollupLoaded,
    truncatedAt:
