...
const ReportsCard = ({ actions }: ReportsCardProps) => {
  ...
  const reportActions = useMemo(() => {
    ...
    const projectReportActions = displayableActions.filter((a: ActionDto) => a.targetableType === "projectReports");
    ...
    projectReportActions.forEach((action: ActionDto) => {
    ...
    siteAndNurseryByTask.forEach(
      (
        { siteReports, nurseryReports }: { siteReports: ActionDto[]; nurseryReports: ActionDto[] },
        taskUuid: string
      ) => {
        const hasProjectReportForTask = projectReportActions.some((a: ActionDto) => {
