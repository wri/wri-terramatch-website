...
/** Filters actions to only those with displayable report status (due, started, needs-more-information) */
...
export function groupSiteAndNurseryReportsByTask(actions: ActionDto[]) {
  ...
  actions
    .filter(a => a.targetableType === "siteReports")
    .forEach(action => {
      const target = action.target as ReportActionTarget;
      const taskUuid = getTaskUuid(target);
      if (!taskUuid) return;
      const existing = map.get(taskUuid) ?? { siteReports: [], nurseryReports: [] };
      existing.siteReports.push(action);
      map.set(taskUuid, existing);
    });

  actions
    .filter(a => a.targetableType === "nurseryReports")
    .forEach(action => {
      const target = action.target as ReportActionTarget;
      const taskUuid = getTaskUuid(target);
      if (!taskUuid) return;
