...
const normalizeOrganisationObject = (item: V2AdminUserRead) => ({
  ...item,
...
export const userDataProvider: UserDataProvider = {
  ...
  async getList(_, params) {
    try {
      ...
      return {
        ...result,
        data: result.data?.map((item: V2AdminUserRead) => normalizeOrganisationObject(item))
      };
    } catch (err) {
  ...
  async getOne(_, params) {
    try {
      ...
      //@ts-ignore
      return { data: normalizeOrganisationObject(response.data) };
    } catch (err) {
  ...
  async getMany(_, params) {
    try {
      ...
      //@ts-ignore
      return { data: response.data?.map(item => normalizeOrganisationObject(item)) };
    } catch (err) {
