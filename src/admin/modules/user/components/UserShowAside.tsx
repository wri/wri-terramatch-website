...
import { usePatchV2AdminUsersVerifyUUID, usePostAuthReset, usePostUsersResend } from "@/generated/apiComponents";
...
export const UserShowAside = () => {
  ...
  const { mutate: resendVerificationEmail } = usePostUsersResend({
    onSuccess() {
      ...
      <Aside title="User Review">
        <Grid container spacing={2} marginY={2}>
          ...
          <Grid item xs={6}>
            ...
            <SelectField source="user_type" choices={userTypesChoices} emptyText="Not Provided" />
          </Grid>
          <Grid item xs={6}>
        ...
        <Box pt={2}>
          <Stack direction="row" alignItems="center" gap={2} flexWrap="wrap">
            <Button
              ...
              onClick={() =>
                resendVerificationEmail({
                  body: { uuid, callback_url: window.location.origin + "/auth/verify/email/" }
                })
              }
            >
            ...
            {record && <DeleteWithConfirmButton confirmTitle="Delete User" />}
          </Stack>
        </Box>
      </Aside>
      <ResetPasswordDialog
