...
export const LevelSelect = (props: LevelSelectProps & { levels: number[] }) => {
  const translate = useTranslate();
  const editor = useTiptapEditor();
  const [anchorElement, setAnchorElement] = useState<HTMLDivElement | null>(null);
  const { size } = props;

  const options = props.levels
    ? allOptions.filter(option => option.value === "paragraph" || props.levels.includes(option.level))
    : allOptions;
...
const allOptions: Array<LevelOption | HeadingLevelOption> = [
  {
