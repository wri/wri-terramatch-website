...
export const useOrgFormDetails = (orgUuid?: string) => {
  ...
  return useMemo<[boolean, OrgFormDetails]>(() => {
    ...
    if (!loaded || orgLoading) return [false, {}];
    return [
      true,
      {
        uuid: orgData?.uuid,
        currency: orgData?.currency ?? undefined,
        startMonth: orgData?.finStartMonth ?? undefined,
        type: orgData?.type ?? undefined
      }
    ];
  }, [orgUuid, loaded, orgLoading, orgData?.currency, orgData?.finStartMonth, orgData?.type, orgData?.uuid]);
};
