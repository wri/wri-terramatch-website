...
const SitePolygonsTabContent: FC<SitePolygonsTabProps> = ({ site }) => {
  ...
  const pendingBulkDeleteUuidsRef = useRef<string[]>([]);
  const pendingBulkSubmitUuidsRef = useRef<string[]>([]);
  const pendingBulkSubmitNamesRef = useRef<string[]>([]);
  const pendingBulkEditUuidsRef = useRef<string[]>([]);
  const [deleteModalPolygons, setDeleteModalPolygons] = useState<PolygonTableRow[]>([]);
  const [bulkEditPolygons, setBulkEditPolygons] = useState<PolygonTableRow[]>([]);
  const [submitModalCounts, setSubmitModalCounts] = useState({ eligible: 0, total: 0 });
  ...
  const handleOpenDeletePolygonModal = useCallback(() => {
    setDeleteModalPolygons(selectedRows);
    pendingBulkDeleteUuidsRef.current = selectedSitePolygonUuids;
  ...
  const handleBulkDelete = useCallback(async () => {
    const sitePolygonUuids = pendingBulkDeleteUuidsRef.current;
  ...
  }, [closeMapPopups, invalidatePolygonMapTiles, openNotification, refetchPolygons, t, toastLabels]);
  ...
  const handleOpenSubmitPolygonsModal = useCallback(() => {
    ...
    pendingBulkSubmitUuidsRef.current = selectedSubmittablePolygonUuids;
    pendingBulkSubmitNamesRef.current = selectedSubmittablePolygons.map(
      polygon => polygon.name ?? t("Unnamed polygon")
    );
    setSubmitModalCounts({
      eligible: selectedSubmittablePolygons.length,
      total: selectedSitePolygons.length
    });
    clearBulkTableSelection();
  ...
  const handleConfirmBulkSubmit = useCallback(async () => {
    const submittablePolygonUuids = pendingBulkSubmitUuidsRef.current;
    if (submittablePolygonUuids.length === 0) {
      openNotification("error", t("Error!"), t("No selected polygons are eligible for submission"));
      return;
    }

    const submittedNames = pendingBulkSubmitNamesRef.current;
  ...
  const handleBulkEditDetails = useCallback(() => {
    ...
    setBulkEditPolygons(selectedRows);
    pendingBulkEditUuidsRef.current = selectedSitePolygonUuids;
    clearBulkTableSelection();
  ...
  const handleBulkEditSave = useCallback(
    async (attributeChanges: BulkSitePolygonAttributeChanges) => {
      const sitePolygonUuids = pendingBulkEditUuidsRef.current;
      if (sitePolygonUuids.length === 0) {
    ...
    <>
      ...
      <PageContent className="bg-theme-neutral-100">
        ...
        <PolygonBulkEditDrawer
          selectedPolygons={bulkEditPolygons}
        ...
        <SubmitPolygons
          ...
          eligibleCount={submitModalCounts.eligible}
          totalCount={submitModalCounts.total}
