...
const SitePolygonsTab: FC<SitePolygonsTabProps> = ({ site, refetch }) => {
  ...
  const { selectedRows, handleRowSelected, onAllItemsSelected } = useTableSelection<PolygonTableRow>(true, mockedData);
  ...
  const restorationPracticeMap = (restorationPractice: restorationStrategyType[]) => {
    ...
    return (
      <Flex className="items-center gap-2">
        {restorationPractice.map((rp, index) => (
          <>
            {SITE_RESTORATION_STRATEGY_MAP[rp as restorationStrategyType]}
    ...
    return (
      <TableRow {...(rowProps as any)} aria-selected={isSelected}>
        ...
        <TableCell className="min-w-[12.5rem] max-w-[17.5rem]">
          <Flex className="items-center gap-2">{restorationPracticeMap(row.restorationPractice)}</Flex>
        </TableCell>
