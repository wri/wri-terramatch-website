...
const PolygonEditDrawer: FC<PolygonEditDrawerProps> = ({
  ...
  const localFinalFocusRef = useRef<HTMLDivElement>(null);
  ...
  const getStableFinalFocusEl = useCallback(() => finalFocusEl?.() ?? localFinalFocusRef.current, [finalFocusEl]);
  ...
  return (
    <>
      <Box ref={localFinalFocusRef} tabIndex={-1} position="fixed" width="1px" height="1px" opacity={0} />
      <Drawer
        modal={false}
      ...
      <SavePolygon
        ...
        restoreFocus={false}
        finalFocusEl={getStableFinalFocusEl}
