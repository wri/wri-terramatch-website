...
interface DisturbanceReportDetailsTabProps {
...
const DisturbanceReportDetailsTab: FC<DisturbanceReportDetailsTabProps> = ({ report }) => {
  const { steps, defaultValues, fieldsProvider, isReady, feedbackBaselineValues } = useEntityFormSetup(
    "disturbance-reports",
    report.uuid
  );
  const { orgDetails, isLoading: orgLoading } = useProjectOrgFormData("disturbance-reports", report);
...
export default DisturbanceReportDetailsTab;
