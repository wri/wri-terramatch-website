...
interface SrpReportDetailsTabProps {
...
const SrpReportDetailsTab: FC<SrpReportDetailsTabProps> = ({ report }) => {
  const { steps, defaultValues, fieldsProvider, isReady, feedbackBaselineValues } = useEntityFormSetup(
    "srp-reports",
    report.uuid
  );
  const { orgDetails, isLoading: orgLoading } = useProjectOrgFormData("srp-reports", report);
...
export default SrpReportDetailsTab;
